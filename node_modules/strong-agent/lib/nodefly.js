...
Nodefly.prototype.profile = function (key, appName, options) {
  ...
  this.info('strong-agent profiling app \'%s\' on host \'%s\'',
    this.appName, this.hostname);
...
Nodefly.prototype.prepareClusterControls = function () {
  ...
  // XXX(sam) strong-agent hooks require... can it emit an event when modules
  // are required, so we can hook and find cluster-control from anywhere? Among
  // other things, this will require cluster-control even if its not being used,
  // but perhaps is just present.
  ...
  if (control) {
    ...
    if (semver.gte(version, '0.2.0')) {
      ...
      this.info(
        'strong-agent cannot use strong-cluster-control v%s,',
        version,
        'please update to >= 0.2.0');
    }
  } else {
    this.info('strong-agent not using strong-cluster-control:', er.message);
  }
...
Nodefly.prototype.info = function () {
  if (!this.quiet) {
